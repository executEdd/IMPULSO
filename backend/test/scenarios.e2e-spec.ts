...
describe('Complex Scenarios & Tiers (e2e)', () => {
  ...
  afterAll(async () => {
    ...
    const studentIds = testUsers.filter((u) => u.studentProfile).map((u) => u.studentProfile!.id);
    const parentIds = testUsers.filter((u) => u.parentProfile).map((u) => u.parentProfile!.id);
    const teacherIds = testUsers.filter((u) => u.teacherProfile).map((u) => u.teacherProfile!.id);

    if (studentIds.length > 0) {
      await prisma.alert.deleteMany({ where: { studentId: { in: studentIds } } });
    ...
    if (schoolCycleId) await prisma.schoolCycle.delete({ where: { id: schoolCycleId } });
  ...
  it('1. should verify Student Life-Cycle (create parent, create student, login, generate QR, register attendance)', async () => {
    ...
    const parentProfileId = parentRes.body.parentProfile?.id || (await prisma.parentProfile.findFirst({
    ...
    const studentProfileId = studentRes.body.studentProfile?.id || (await prisma.studentProfile.findFirst({
  ...
  it('2. should verify Red Semaphore Trigger and Reset (3 absences trigger RED, alert created, admin reset to GREEN)', async () => {
    ...
    const absentRes = await request(app.getHttpServer())
      .post(`/api/attendance/mark-absent/${studentProfileId}/${cl.schedules[0].id}`)
      .set('Authorization', `Bearer ${teacherAToken}`);
  ...
  it('3. should verify Double-period class smart scan (prevents duplicate scans on same day)', async () => {
    ...
    if (hasClassScheduleId) {
      ...
      expect(scanRes3.body.message).toBe('La asistencia de este alumno ya fue registrada para esta clase hoy');
    } else {
      // If Milestone 4 is not active yet, scan for another schedule for same class today fails
      expect(scanRes2.status).toBe(400);
      expect(scanRes2.body.message).toBe('La asistencia de este alumno ya fue registrada para esta clase hoy');
    }
  });
  ...
  it('4. should verify Teacher shift cover (different teacher rejected, admin updates teacher, scan succeeds)', async () => {
    ...
    expect(scanFail.body.message).toBe('No está autorizado para registrar asistencia en esta clase. El docente no coincide con el horario asignado.');
