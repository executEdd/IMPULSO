...
import { ApiTags, ApiOperation, ApiBearerAuth, ApiQuery, ApiOkResponse, ApiCreatedResponse, ApiBadRequestResponse } from '@nestjs/swagger';
...
export class AttendanceController {
  ...
  @ApiCreatedResponse({ description: 'Asistencia registrada con éxito (Smart-Check completado).' })
  @ApiBadRequestResponse({ description: 'QR inválido, expirado o error al asociar el docente.' })
  async scanQr(
  ...
  @ApiCreatedResponse({ description: 'Falta registrada manualmente con éxito.' })
  @ApiBadRequestResponse({ description: 'Error en la petición o IDs incorrectos.' })
  async markAbsent(
  ...
  @ApiOkResponse({ description: 'Listado de asistencias recuperado exitosamente.' })
  async findAll(
  ...
  @ApiOkResponse({ description: 'Historial completo de asistencias del alumno recuperado.' })
  ...
  @ApiOkResponse({ description: 'Conteo numérico de faltas en los últimos 30 días.' })
  ...
  @ApiOkResponse({ description: 'Listado de estudiantes con estatus de semáforo en Rojo debido a inasistencias.' })
